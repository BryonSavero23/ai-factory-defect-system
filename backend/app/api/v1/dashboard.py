...
def get_dashboard_summary(session: Session = Depends(get_session)):
    ...
    total_defects = session.exec(
        select(func.count(Inspection.id))
        .where(Inspection.status == "DEFECT")
    ).one()

    total_pass = session.exec(
        select(func.count(Inspection.id))
        .where(Inspection.status == "PASS")
    ).one()
...
@router.get(
    "/defect-breakdown",
    response_model=list[DefectBreakdownItem]
)
...
@router.get(
    "/machine-breakdown",
    response_model=list[MachineBreakdownItem]
)
